...
public class LoginPage extends Application {
    ...
    private void handleLogin() {
        ...
        if (user != null && user.getPassword().equals(password) && user.getRole().equalsIgnoreCase(selectedUserType)) {
            ...
            // Pass the user object to the relevant page based on user type
            if (selectedUserType.equalsIgnoreCase("Admin")) {
                controller.showViewArticlesPage(user);
            } else {
                controller.showListOfGroupsPage(user); // Redirect to ListOfGroupsPage for non-admin users
            }
        } else {
            labelMessage.setTextFill(Color.RED);
            labelMessage.setText("Invalid credentials or user type.");
        }
    }
